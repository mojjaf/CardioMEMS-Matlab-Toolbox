function [AO_amp_raw,AO_i_raw,heartrate_avg,t]=APD2(data,fs,coef,flag,gr)
...
sig_comb_int=filter([1/15, 1/15, 1/15],1,sig_comb_int);
...
[pks,locs]=findpeaks(sig_comb_m,'MinPeakHeight',(max(sig_comb_m(1:fs*10))*0.25), 'MinPeakDistance',fs*0.37);
...
AO_amp_raw(b)=[];
AO_i_raw(b)=[];
[aa,bb]=find(diff(AO_i_raw)<=0.35*fs);
AO_amp_raw(bb)=[];
AO_i_raw(bb)=[];
