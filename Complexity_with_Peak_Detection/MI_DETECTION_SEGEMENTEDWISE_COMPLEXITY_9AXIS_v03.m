...
%                          hfiles = 'D:\BIOSIGNAL_PROCESSING\MI Detect project\STEMI_Mechanical Complexity\DATA_CLEANED_04092018\NEW CLEANED _ 15012019\CONTROLS_HIGHQUALITY_SELECTED/';
            %             STEMI Control
                        hfiles = 'D:\BIOSIGNAL_PROCESSING\STENOSIS STUDY\SAFE STEMI\New CLEANED_ byOLLI\Control/';
            %             STEMI post
...
%         
        ...
        jitter=floor(min(length(data.data_cleaned.accX_lsm)/fs_lsm,length(data.data_cleaned.ADXL355_hf.accX)/fs_adxl));
        
        gyroX=fft_filter(data.data_cleaned.gyroX(1:jitter*fs_lsm),fs_lsm,3,30);
        gyroY=fft_filter(data.data_cleaned.gyroY(1:jitter*fs_lsm),fs_lsm,3,30);
        gyroZ=fft_filter(data.data_cleaned.gyroZ(1:jitter*fs_lsm),fs_lsm,3,30);
        accX=fft_filter(data.data_cleaned.accX_lsm(1:jitter*fs_lsm),fs_lsm,5,30);
        accY=fft_filter(data.data_cleaned.accY_lsm(1:jitter*fs_lsm),fs_lsm,5,30);
        accZ=fft_filter(data.data_cleaned.accZ_lsm(1:jitter*fs_lsm),fs_lsm,5,30);
        ekg_lf=fft_filter(data.data_cleaned.ekg(1:jitter*fs_lsm),fs_lsm,0.5,40);
        
        data_lsm=struct('ekg',ekg_lf,'accX',accX,'accY',accY,'accZ',accZ,'gyroX',gyroX,'gyroY',gyroY,'gyroZ',gyroZ);
        
        
        
        accX_hf=fft_filter(data.data_cleaned.ADXL355_hf.accX(1:jitter*fs_adxl),fs_adxl,30,80);
        accY_hf=fft_filter(data.data_cleaned.ADXL355_hf.accY(1:jitter*fs_adxl),fs_adxl,30,80);
        accZ_hf=fft_filter(data.data_cleaned.ADXL355_hf.accZ(1:jitter*fs_adxl),fs_adxl,30,80);
        accX_lf=fft_filter(data.data_cleaned.ADXL355_hf.accX(1:jitter*fs_adxl),fs_adxl,1,30);
        accY_lf=fft_filter(data.data_cleaned.ADXL355_hf.accY(1:jitter*fs_adxl),fs_adxl,1,30);
        accZ_lf=fft_filter(data.data_cleaned.ADXL355_hf.accZ(1:jitter*fs_adxl),fs_adxl,1,30);
        
        accZ_hpf= baseline_removal_imu(data.data_cleaned.ADXL355_hf.accZ,1.5,fs_adxl);
        ekg_hf=resample(ekg_lf,fs_adxl,fs_lsm);
...
% %        close all
            figure
            ax(1)=subplot(211); plot(data_adxl.ekg)
            
            ax(2)=subplot(212);plot(envelope(data_adxl.accZLF.^2))
            hold on
            plot(envelope(data_adxl.accZ.^2))
            plot(envelope(accZ_hpf.^2))
            legend('AccZ 1-30Hz','AccZ 30-80Hz','AccZ hpf F_c=1.5Hz');
